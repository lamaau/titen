  ...
  <ul class="space-y-2 pb-2">
    <li v-for="(navigator, index) in JSON.parse(navigators)" :key="index">
      <v-app-link
        ...
        :class="{
          'bg-emerald-100 text-emerald-500':
            route().has(navigator.url) && currentUrl === navigator.url
              ? true
              : false,
        }"
      ...
      <a
        ...
        class="p-2 flex group rounded-lg font-normal items-center transition-all hover:bg-emerald-100 hover:text-emerald-500 text-sm text-gray-900"
        :class="{
          'bg-emerald-100 text-emerald-500': navigator.subItems.filter(
            (v) => v.url === currentUrl,
          ).length,
        }"
      >
              ...
              <v-app-link
                ...
                :class="{
                  'text-emerald-500 font-bold font-sans':
                    route().has(subNavigator.url) &&
                    currentUrl === subNavigator.url
                      ? true
                      : false,
                }"
