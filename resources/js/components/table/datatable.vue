<template>
  <div class="border border-gray-200 shadow dark:border-cool-gray-700 sm:rounded-lg">
    <div class="p-4" v-if="title">
      <h2 class="text-xl font-bold text-gray-700 dark:text-cool-gray-300">{{ title }}</h2>
    </div>

    <div class="flex items-center border-t border-dashed bg-white p-5 dark:border-cool-gray-700 dark:bg-cool-gray-600" :class="[$slots['attributes'] ? 'justify-between' : 'justify-end']">
      ...
      <div class="flex items-center space-x-2">
        ...
        <select v-model="params.perpage" v-if="allowPerPage" class="form-select text-sm dark:border-cool-gray-500 dark:bg-cool-gray-700 dark:text-cool-gray-300">
    ...
    <div class="overflow-auto border-b border-dashed dark:border-cool-gray-700">
      <div class="flex space-x-6 border-t border-dashed py-2 px-4 dark:border-cool-gray-700" v-if="params.column && params.direction">
        <div class="flex items-center space-x-2">
          <p class="text-sm dark:text-cool-gray-200">Penyortiran Diterapkan:</p>
          <span class="flex items-center rounded-md bg-purple-500 text-sm text-white">
            <p class="py-1 px-2">{{ columnName }}: {{ params.direction === "asc" ? "A-Z" : "Z-A" }}</p>
            <button @click.prevent="handleClearSort" type="button" class="mr-1 rounded-md bg-purple-600 py-1 px-1">
      ...
      <table class="min-w-full divide-y divide-gray-200 dark:divide-cool-gray-500">
            ...
            <template v-for="(column, index) in columns" :key="index">
              <th v-if="!column.blank" scope="col" class="table-heading-cell">
                <div class="flex items-center" @click="handleSort(column)" :class="[column.sortable ? 'cursor-pointer' : 'cursor-default']">
                  ...
                  <v-icon name="ChevronUpIcon" type="solid" v-if="filters && column.sortable && params.direction == 'asc' && params.column == column.column" class="ml-2 h-4 w-4" />
                  <v-icon name="ChevronDownIcon" type="solid" v-if="filters && column.sortable && params.direction == 'desc' && params.column == column.column" class="ml-2 h-4 w-4" />
                </div>
              </th>
            </template>
          </tr>
        </thead>
        <tbody v-if="data.data.length" class="divide-y divide-gray-200 bg-white dark:divide-cool-gray-700">
        ...
        <tbody v-else>
          <tr>
            <td :colspan="columns.filter((c) => c.blank === false).length" class="bg-white py-8 text-center text-gray-400 dark:bg-cool-gray-600 dark:text-cool-gray-200">
...
export default {
  ...
  async mounted() {
    if (this.filters && this.filters.filters !== null) {
      ...
      let response = await Promise.all(keys.map((value) => fetch(`${process.env.MIX_APP_URL}/${this.fields[value].url}/${this.filters.filters[value]}`)))
